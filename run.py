...
def login():
    ...
    if "mbasic_logout_button" in log:
       if "Apa yang Anda pikirkan sekarang" in log:
          try:
              open("cookies","w").write(cokie["cookie"])
          except:
              pass
       else:
          try:
              lg=bs(log,"html.parser").find("form",action=lambda x: "/intl/save_locale/?loc=id_ID" in x)
              dt=lg.find_all("input",type="hidden")
              ses.post(mbasic.format(lg["action"]),data={"fb_dtsg":dt[0]["value"],"jazoest":dt[1]["value"]},cookies=cokie)
          except:
              pass
       try:
...
def pilih_menu():
    ...
    if choice == "00" or choice == "0":
       ...
       print(f"\r{er}Please Waiting...",end="")
       os.system("rm cookies")
       os.system("rm useragent")
       ses.headers.clear()
       sleep(1)
       print(f"\r{dn}Done.        ",end="")
       print()
       print(f"{ab}-----------------------------------------------")
       cblg()
    else:
...
if __name__=="__main__":
   ...
   try:
       ...
       ses.get("https://mbasic.facebook.com/data/manifest/",cookies=cokie)
       ses.get("https://facebook.com/security/hsts-pixel.gif",cookies=cokie)
       menu()
