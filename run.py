...
def login():
    try:
        ...
        user=input(f"{pr}Username/ID : {rgb}")
        pw=input(f"{pr}Password    : {rgb}")
        req=ses.get("https://mbasic.facebook.com/login/?next&ref=dbl&fl&refid=8",headers={"Host":"mbasic.facebook.com","cache-control":"max-age=0","upgrade-insecure-requests":"1","user-agent":ua,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3","referer":"https://mbasic.facebook.com/","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"}).text
        url=bs(req,"html.parser").find("form",action=lambda x: "/login/device-based/" in x)
        dt={}
        for x in url.find_all("input"):
            if x.get("name") in ["lsd","jazoest","m_ts","li","try_number","unrecognized_tries","email","pass","login","bi_xrwh"]:
               dt.update({x.get("name"):x.get("value")})
            else:
               continue
        dt.update({"email":user,"pass":pw})
        log=ses.post(mbasic.format(url["action"]),data=dt,headers={"Host":"mbasic.facebook.com","cache-control":"max-age=0","origin":"https://mbasic.facebook.com","upgrade-insecure-requests":"1","content-type":"application/x-www-form-urlencoded","user-agent":ua,"accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3","referer":"https://mbasic.facebook.com/login/?next&ref=dbl&fl&refid=8","accept-language":"id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7"})
        if "checkpoint" in str(log.url):
           exit(f"{er}Account CheckPoint")
        elif "save-device" in str(log.url):
           cokie=ses.cookies.get_dict()
           ck=f'datr={cokie["datr"]};sb={cokie["sb"]};c_user={cokie["c_user"]};xs={cokie["xs"]};fr={cokie["fr"]}'
           with open("cookies","w") as ex:
                ex.write(ck)
        else:
           print(f"{er}Wrong username/password")
           sleep(2)
           login()
    cokie={"cookie":ck}
    try:
        bhs=bs(ses.get(mbasic.format("/me"),cookies=cokie).text,"html.parser").find("form",action=lambda x: "/intl/save_locale/?loc=id_ID" in x)
        ind={}
    ...
    try:
        ...
        pass
    try:
        hm=ses.get(mbasic.format("/me"),cookies=cokie).text
        name=bs(hm,"html.parser").find("title").text
...
if __name__=="__main__":
   ...
   try:
       ...
       ua=input(f"{er}Put your useragent\n{pr} {ab}>>> {rgb}")
   try:
